...
4. **代码质量**：确保修改后代码可直接运行
5. **禁止 href 导航**：所有页面/状态切换必须用 `@click="currentState = '...'"` 实现，不要使用 `<a href="/">`
...
## 输出格式
请按以下格式输出：
1. **先简要说明**你做了哪些修改（1-2 句话），让用户了解改动内容
2. 然后输出修改后的完整 HTML 代码，使用 markdown 代码块包裹

示例：
根据您的需求，我将按钮颜色从灰色改为蓝色，并调整了圆角大小。
